...
function integer MYHDL26_get_position;
    input [24-1:0] addr;
    input page_size;
    integer page_size;
begin: MYHDL38_RETURN
    MYHDL26_get_position = (addr / page_size);
    disable MYHDL38_RETURN;
end
endfunction

function integer MYHDL27_get_configuration;
    input [3-1:0] mem;
begin: MYHDL39_RETURN
    MYHDL27_get_configuration = (mem & 1);
    disable MYHDL39_RETURN;
end
endfunction

function integer MYHDL28_get_encode;
    input [3-1:0] mem;
begin: MYHDL40_RETURN
    MYHDL28_get_encode = (mem >>> 1);
    disable MYHDL40_RETURN;
end
endfunction

function MYHDL29_decoder_check;
    input [1-1:0] data;
    input type;
    integer type;
begin: MYHDL41_RETURN
    MYHDL29_decoder_check = 1'b1;
...
function integer MYHDL31_get_next_encode;
    input enc;
    integer enc;
    integer LAST_ENCODE;
begin: MYHDL43_RETURN
    ...
    if ((enc >= 3)) begin
        ...
        $write("%0d", enc);
        $write("\n");
        MYHDL31_get_next_encode = (enc + 1);
        disable MYHDL43_RETURN;
    end
end
endfunction

function [16-1:0] MYHDL32_decoder;
    input [16-1:0] data;
    input type;
    integer type;
...
function [16-1:0] MYHDL33_decoder;
    input [16-1:0] data;
    input [3-1:0] type;
...
function [16-1:0] MYHDL35_decoder;
    input [16-1:0] data;
    input [3-1:0] type;
begin: MYHDL46_RETURN
    MYHDL35_decoder = data;
    disable MYHDL46_RETURN;
end
endfunction

function [16-1:0] MYHDL36_encoder;
    input [16-1:0] data;
    input [3-1:0] type;
begin: MYHDL47_RETURN
    MYHDL36_encoder = data;
    disable MYHDL47_RETURN;
end
endfunction

task MYHDL37_set_encode;
    output [3-1:0] mem;
    input [3-1:0] enc;
begin: MYHDL48_RETURN
    mem[1] <= (enc & 1);
    mem[2] <= ((enc & 2) >>> 1);
end
endtask
...
always @(posedge clk_i) begin: DFTM_MAIN
    ...
    if ((current_operation_mode == 2'b00)) begin
        if (host_intf_dftm_i) begin
            ...
            iram_current_position = MYHDL26_get_position(host_intf_addr_i, 256);
            current_encode = 0;
            is_dynamic = 0;
            ...
            if ((iram_current_position < 256)) begin
                ...
                is_dynamic = MYHDL27_get_configuration(ram_inf);
                is_dynamic = 1;
                current_encode = MYHDL28_get_encode(ram_inf);
            end
            ...
            if (host_intf_sdram_done_o) begin
                if ((in_read == 0)) begin
                    ...
                    in_read <= 0;
                    decode_ok = MYHDL29_decoder_check(host_intf_sdram_done_o, current_encode);
                    if ((host_intf_addr_i == 120)) begin
                        decode_ok = 0;
                    end
                    if (decode_ok) begin
                        host_intf_data_o <= MYHDL30_decoder(host_intf_sdram_data_o, current_encode);
                        host_intf_done_o <= host_intf_sdram_done_o;
                    end
                    else begin
                        next_encode = MYHDL31_get_next_encode(current_encode);
                        ...
                        if (recode) begin
                            ...
                            host_intf_data_o <= MYHDL32_decoder(host_intf_sdram_data_o, current_encode);
                            host_intf_done_o <= host_intf_sdram_done_o;
                            in_read <= 0;
                        end
                    end
        ...
        if ((current_recoding_mode == 3'b001)) begin
            ...
            if (host_intf_sdram_done_o) begin
                ...
                recode_data_o <= MYHDL33_decoder(host_intf_sdram_data_o, recode_from_ecc);
                recode_data_o <= (MYHDL35_decoder(host_intf_sdram_data_o, recode_from_ecc) + 1);
        ...
        if ((current_recoding_mode == 3'b010)) begin
            ...
            $write("\n");
            host_intf_sdram_addr_i <= recoding_current_address;
            host_intf_sdram_data_i <= MYHDL36_encoder(recode_data_o, recode_to_ecc);
        ...
        if ((current_recoding_mode == 3'b011)) begin
            ...
            if (host_intf_sdram_done_o) begin
                ...
                if ((r_count < 256)) begin
                    ...
                    MYHDL37_set_encode(ram_inf, recode_to_ecc);
