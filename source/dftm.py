...
@block
def dftm(clk_i, ext_intf, host_intf_sdram, dftm_iram_page_size = 256):
    ...
    def main():        
        if current_operation_mode == OPERATION_MODE.NORMAL:
            if ext_intf.dftm_i:
                ...
                host_intf_sdram.addr_i.next = ext_intf.addr_i            
                host_intf_sdram.rd_i.next = ext_intf.rd_i
                host_intf_sdram.wr_i.next = ext_intf.wr_i
                host_intf_sdram.data_i.next = ext_intf.data_i
                #print_sig("r",host_intf_sdram)
                if host_intf_sdram.rdPending_o == 1:
                    in_read.next = 1
                ...
                if host_intf_sdram.done_o:
                    if in_read == 0:
                        ...
                        in_read.next = 0                    
                        decode_ok = ecc.check(host_intf_sdram.data_o, current_encode)
                        ...
                        if decode_ok:
                            ext_intf.data_o.next = ecc.decode(host_intf_sdram.data_o, current_encode)
                            ext_intf.done_o.next = host_intf_sdram.done_o
                        else:                        
            ...
            if current_recoding_mode == RECODING_MODE.WAIT_READ:
                ...
                if host_intf_sdram.done_o:
                    ...
                    decoded_data = ecc.decode(host_intf_sdram.data_o, recode_from_ecc)
                    recode_data_o.next = decoded_data
                    #debug propose
                    recode_data_o.next = ecc.decode(host_intf_sdram.data_o, recode_from_ecc) + 1
            ...
            if current_recoding_mode == RECODING_MODE.WRITE:
                ...
                host_intf_sdram.addr_i.next = recoding_current_address
                #recode_current_ecc
                host_intf_sdram.data_i.next = ecc.encode(recode_data_o, recode_to_ecc)
